angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  //that will call the method on the itunesService that is responsible for getting the data from iTunes, whenever the user clicks the submit button
  ...
  $scope.getArtist = function(artist) {
    itunesService.getArtist(artist).then(function(resultFromService) {
      $scope.songData = resultFromService;
    })
