angular.module('itunes').service('itunesService', function($http, $q){
  ...
  //Write a method that 
  //accepts an artist's name as the parameter, 
    ...
    this.getArtist = function(artist) {
      ...
      .then(function(response) {
        ...
        for (var i = 0; i < results.length; i++) {
          ...
          var newRes = new newResultsFormat(results[i].artworkUrl100, results[i].artistName, results[i].trackCensoredName, results[i].collectionName, results[i].collectionPrice, results[i].previewUrl, results[i].kind)
